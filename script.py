...
# --- 1. 配置和环境变量部分 (无新增，但请注意 COUNTRY_ORDER 的用法已改变) ---
GITHUB_TOKEN = os.getenv("MY_GITHUB_TOKEN")
REPO_NAME = os.getenv("REPO_NAME")
FILE_PATH = os.getenv("FILE_PATH")
WEBPAGE_URLS = os.getenv("WEBPAGE_URLS", "").strip().splitlines()

# 重要：这里的顺序决定了最终输出的节点顺序。请根据您源文件中实际的代码（如 HKG, NRT, ICN, US 等）来填写。
COUNTRY_ORDER_STR = os.getenv("COUNTRY_ORDER") or "HKG,NRT,ICN,HK,SG,JP,TW,KR,US"
...
def extract_vless_links(decoded_content):
    """解析 VLESS/VMESS 等 Base64 编码的订阅链接。"""
    # 国家/地区名称到代码的映射 (主要用于 VLESS 的中文名)
    COUNTRY_MAPPING = {
...
def extract_csv_links(csv_content):
    """解析 CSV 格式的内容。"""
    links = []
    # 使用 StringIO 将字符串模拟成文件，交给 csv 模块处理
    ...
    try:
        ...
        for row in reader:
            if len(row) >= 4:
                ip = row[0].strip()
                port = row[1].strip()
                code = row[3].strip() # 第4列是数据中心代码, 如 HKG
...
# <<< 新增功能：支持 Trojan 和 任意代码 的行解析器 >>>
def extract_line_based_links(plain_content):
    """解析各种以行为单位的链接格式 (Trojan, IP:Port#自定义名)。"""
    links = []
    for line in plain_content.strip().splitlines():
        clean_line = line.strip()
        if not clean_line: continue
        
        # 优先匹配 Trojan 格式: trojan://<...stuff...>@<host>:<port>#<name>
...
# --- 4. 排序和写入 GitHub 的函数 (有微小修改) ---
def filter_and_sort_links(all_links, order, limit):
    grouped_links = {}
    for link_info in all_links:
        # 注意：这里的 key 从 "country_code" 改为更通用的 "code"
        code = link_info['code']
